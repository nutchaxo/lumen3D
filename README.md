...
*   **Multi-Panel Compare**: Side-by-side dataset comparison (`compare.html`) — volumes and photographs in the same board, each panel the real viewer page embedded chrome-less and driven by `postMessage`: camera / slicer-plane / z-stack / time / channel sync, photographs linked by physical view (µm per screen pixel), one tool for every panel, the panel-capable plugins (`plugin.json#contexts`) drawn as per-panel buttons, staged render quality, a dataset picker with search and type filter, workspace save/restore and a Slice Studio over all panels at visual or physical scale.
