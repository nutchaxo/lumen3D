...
| Compare | `compare.html` | [compare.js](js/pages/compare.js) | Side-by-side panels via iframes of `viewer.html` (volumes) or `wholemount.html` (photographs, linked by physical view `WM_PHYSICAL_VIEW`, since v1.50.0) |
| Tracking | `tracking.html` | [tracking.js](js/pages/tracking.js) | Cell tracking timelapse viewer |
| Wholemount | `wholemount.html` | [wholemount.js](js/pages/wholemount.js) | **2D photograph viewer** (type `wholemount`, since v1.47.0) — one calibrated stereomicroscope picture per dataset, pan/zoom canvas ([wholemount-viewer.js](js/viewers/wholemount-viewer.js)), scale bar, X-gal stain isolation, and a contact-sheet browser over every wholemount of the catalog (filter by stage/line, ←/→, in-place switch, preview→native). Plugins load **opt-in** via `plugin.json#dataTypes`. Measurement labels are draggable (`labelOffset` in image px) with a size slider; *Open in Studio* hands the native canvas to `StudioEditor` (`sliceResult.dataset` + `pixelSizeUm{x,y}`); `?mode=admin&hideHeader=true&path=<type/folder or staging:…>` is the admin preview form (v1.48.0). Since v1.49.0 the viewer has three frames (image → oriented → canvas; rotation + mirror via `setOrientation`), display adjustments (`setAdjustments`, display-only), plugin overlays (`addOverlay`), a physical view (`get/setPhysicalView`: µm per CSS px + physical centre) and a pane mode (`?panelIndex=`) exchanging that view with its parent; the plugin ctx adds `ui.addSidebarSection`, `ui.getStage`, `ui.openStudioWith`, `dataset.getCollection/fileUrl/open/onChange`. Wholemount plugins: `orientation-2d`, `calibrated-grid`, `display-adjust`, `split-view`, `figure-panel` (+ screenshot, presentation-mode, download-center, measure-distance). |
...
| — | [wholemount_importer.py](preprocess/wholemount_importer.py) | **Photographs, standalone** (v0.17.0). One ImageJ/Leica TIFF → one `DATA_WEB/wholemount/<name>/` dataset: `image.webp` (native, q90), `preview.webp` (640 px), `thumbnail.webp`, `metadata.json` (type `wholemount`, `image{}`, `pixelSizeUm` from the TIFF resolution tags, `acquisition{}` from the Leica block, stage/zoom/dissection date from the file name), optional `download/` with the original TIFF. `--force` re-imports but keeps lab-curated keys (`merge_curated`). `python wholemount_importer.py --input <dir> --output DATA_WEB --staining X-gal --with-downloads`. |
...
└── wholemount/<dataset>/       # ONE 2D photograph (no bricks/): image.webp + preview.webp
                                # + thumbnail.webp + metadata.json (+ download/ original TIFF)
```

**Dataset types** drive UI (filters, viewer behavior) : `fixed`, `live`, `tracking`, `wholemount`. Set by the directory `<type>/` under `DATA_WEB`. The type list lives in `dev_server.py:ALLOWED_TYPE_DIRS`, `upload_staging.py:ALLOWED_TYPE_DIRS` (+ `VOLUME_TYPE_DIRS` for the brick-carrying ones), their PHP twins (`LUMEN_UP_TYPES`, `api/datasets.php:$TYPES`, `_admin_lib.php`, `downloads.php`), and client-side `Utils.DATASET_TYPES` / `Utils.datasetPage()` (the one place that maps a type to its page).
...
| Scope a plugin to dataset types | Declare `"dataTypes": ["fixed", "live", "tracking", "wholemount"]` in its `plugin.json`. The gate in `PluginRegistry.loadModules(base, paths, { dataType, allowUndeclaredDataTypes })` cuts **both ways** (since v1.50.1): a plugin that declares `dataTypes` loads on those types and on **no other** — that is what keeps a photograph-only tool off the volume viewer; a plugin that declares nothing predates the field and is taken only by a host passing `allowUndeclaredDataTypes` (only `viewer.js` does, so its historical + marketplace plugins are unaffected). Left out ≠ quarantined. No type name is hardcoded in the registry — `viewer.js` passes `datasetMeta.type`, `wholemount.js` its own. **Renaming a dataset-type id means renaming every `dataTypes` array in lockstep**, or the declaring plugins vanish from both pages. Vector: `tests/js/test_plugin_datatype_gate.mjs`. Today: `screenshot`, `presentation-mode`, `download-center`, `measure-distance`, and the wholemount-only `orientation-2d`, `calibrated-grid`, `display-adjust`, `split-view`, `figure-panel`. The page's ctx mirrors the viewer's (`dataset/viewer/measurements/ui/workspace`, `ui.getCanvas`) and adds overlays, orientation, adjustments, physical view, sidebar sections and the collection (see the Wholemount row in §2.1). |
