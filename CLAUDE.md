...
| `tools/zstack-browser` | Z-stack browser (v1.1.0, web v1.52.0). Custom vertical slider: a **3D notch** above the track (default; every kept slice, rotation free), the track locks the view top-down and shows the cursor's slices; the cursor is a **bar of adjustable thickness** (drag an edge / *Thickness* stepper); two **triangular trim handles** mask the slices above / below in both modes. One `setClipRange_z` on the volume; the shader confines the march to the clip box + slab-normalises emission (see `volume-viewer.js` `hitClipBox`/`slabGain`) so a single slice renders. `SYNC_ZSTACK_SLICE` carries mode/cursor/thickness/crop; `ctx._state.zstackCurrentSlice` = cursor centre, −1 in 3D. Tests: `tests/js/test_zstack_slab.mjs`, `test_zstack_echo.mjs`. |
...
**Dataset types — one vocabulary, four values.** A type is `3d`, `2d`, `live` or `tracking`, and that single spelling is the directory under `DATA_WEB/` **and** under `uploads/staging/`, the first segment of the dataset id (`dataset.id === dataset.path === '<type>/<folder>'`, the same string in `metadata.json`'s `"id"`), the `"type"` field of `metadata.json`, a value of `plugin.json#dataTypes`, the `<type>` of `staging:<type>/<folder>` and of the journal `uploads/state/<type>__<folder>.json`, the `?type=` explorer filter, and the suffix of `badge-<type>` / `type-card--<type>`. There is **no alias and no legacy spelling** : `fixed` and `wholemount` no longer exist anywhere in the code.
...
| `live` | `DATA_WEB/live/` | `viewer.html` | one brick tree per timepoint |
| `tracking` | `DATA_WEB/tracking/` | `tracking.html` | trajectories (`tracks.json`) over a volume |
...
* …commit datasets in `DATA_WEB/3d|2d|live|tracking/` (large binary, gitignored — only `.gitkeep` is tracked; `catalog.json` is generated, not stored).
* …write `fixed` or `wholemount` as a dataset type again, in code, config, a plugin manifest or a test fixture (§5). And never hardcode a type's *display* name — that is the operator's, via `Utils.datasetTypeLabel()`.
