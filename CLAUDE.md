...
Plugin pattern : each module has `plugin.json` (metadata) + `index.js` (calls `PluginRegistry.implement(id, {init, activate, …})`). Module manifest is hard-coded in [viewer.js](js/pages/viewer.js:150) — when adding a module, append it to that array.
...
1. `viewer.js` lists modules in the manifest array → calls `PluginRegistry.loadModules('js/modules', manifest)`.
...
4. `viewer.js` then calls `PluginRegistry.initAll(ctx)` — passes the `ViewerContext` (`{ VolumeViewer, datasetMeta, … }`) to every module's `init`.
5. UI population : tools panel queries `PluginRegistry.listByPlacement('tools')` etc., sorts by `meta.order`, renders buttons.

**Order matters** : `loadModules` is awaited BEFORE the UI builds its tool/shader/channel lists. Regression in v0.12.45 ([changelog_0.12.45.md](changelog/changelog_0.12.45.md)) — keep this invariant.
...
| Add a new tool | Create `js/modules/tools/<id>/{plugin.json, index.js}` + append `'tools/<id>'` to manifest in [viewer.js:150](js/pages/viewer.js:150) |
