...
| Make a plugin available on the wholemount page | Declare `"dataTypes": ["fixed", "live", "wholemount"]` in its `plugin.json`. `PluginRegistry.loadModules(base, paths, { dataType })` is **opt-in**: when a page passes a `dataType`, a plugin that does not list it is never injected (not a quarantine — just left out). `viewer.js` passes none, so the volume viewer is unchanged. Today: `screenshot`, `presentation-mode`, `download-center`, `measure-distance`. The page's ctx mirrors the viewer's (`dataset/viewer/measurements/ui/workspace`, `ui.getCanvas`). |
