...
def _pack_timepoint(temp_dir: Path, bricks_dir: Path, t_idx: int, lod_levels, n_ch: int,
    ...
    for li in lod_levels:
        ...
        # Empty-space skipping is decided per timepoint: cells move, so the occupied
        # brick set legitimately differs from one frame to the next.
        ...
        print(f"[PACKER] {tp_subdir or 't000'} LOD {lod_num}: Grid {nx}x{ny}x{nz} "
              f"({len(chunks_grid)} chunks, {len(active_chunks_grid)} active after thresholding)")
        ...
        for c_idx in range(n_ch):
            ...
            # executor.map preserves the order of active_chunks_grid
            for result in tqdm(executor.map(process_chunk, tasks), total=len(tasks),
                               desc="Compressing WebP", leave=False, ascii=True, mininterval=2.0):
                ...
                if is_non_empty:
                    ...
                    if chunks_in_current_pack >= CHUNKS_PER_PACK:
                        ...
                        pack_rel_path = pack_file_path.relative_to(tp_root).as_posix()
...
def build_packs(temp_dir: Path, output_dir: Path):
    ...
    # One process pool for the whole run: a timelapse packs n_tp x n_lod x n_ch
    # batches and re-spawning a pool for each would dominate the runtime.
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        if not is_timelapse:
            levels_manifest, transport = _pack_timepoint(
                temp_dir, bricks_dir, 0, lod_levels, n_ch, executor, ""
            )
            timepoints_manifest = None
        else:
            timepoints_manifest = {}
            levels_manifest = None
            transport = None
            for t_idx in range(n_tp):
                key = f"t{t_idx:03d}"
                print(f"[PACKER] === timepoint {t_idx + 1}/{n_tp} ({key}) ===")
                tp_levels, tp_transport = _pack_timepoint(
                    temp_dir, bricks_dir, t_idx, lod_levels, n_ch, executor, key
                )
                timepoints_manifest[key] = {
                    ...
                    "histograms": []   # filled by step 4
                }
                if t_idx == 0:
                    # Mirrored at the top level so a consumer that ignores `timepoints`
                    # still mounts a coherent (first-frame) dataset instead of failing.
                    levels_manifest = tp_levels
    ...
    if is_timelapse:
        size_mb = (bricks_dir / "manifest.json").stat().st_size / 1e6
        print(f"[PACKER] {n_tp} timepoints indexed, manifest {size_mb:.2f} MB")
