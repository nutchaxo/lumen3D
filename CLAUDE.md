...
| Make a plugin available on the wholemount page | Declare `"dataTypes": ["fixed", "live", "wholemount"]` in its `plugin.json`. `PluginRegistry.loadModules(base, paths, { dataType })` is **opt-in**: when a page passes a `dataType`, a plugin that does not list it is never injected (not a quarantine — just left out). `viewer.js` passes none, so the volume viewer is unchanged. Today: `screenshot`, `presentation-mode`, `download-center`, `measure-distance`, and the wholemount-only `orientation-2d`, `calibrated-grid`, `display-adjust`, `split-view`, `figure-panel`. The page's ctx mirrors the viewer's (`dataset/viewer/measurements/ui/workspace`, `ui.getCanvas`) and adds overlays, orientation, adjustments, physical view, sidebar sections and the collection (see the Wholemount row in §2.1). |
