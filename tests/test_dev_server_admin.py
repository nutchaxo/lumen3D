...
class TestStats(unittest.TestCase):
    ...
    def test_counts_under_plural_fields(self):
        ...
        dev_server._record_event("view", "3d/foo")
        dev_server._record_event("view", "3d/foo")
        dev_server._record_event("download", "3d/foo")
        ...
        self.assertEqual(s["datasets"]["3d/foo"]["views"], 2)
        self.assertEqual(s["datasets"]["3d/foo"]["downloads"], 1)
    ...
    def test_visit_is_not_attributed_to_a_dataset(self):
        dev_server._record_event("visit", "3d/foo")  # dataset id ignored for visits
        s = dev_server._load_stats()
        self.assertNotIn("3d/foo", s["datasets"])
...
class TestHiddenCatalog(unittest.TestCase):
    def setUp(self):
        ...
        ds = dev_server.DATA_WEB / "3d" / "Demo"
        ds.mkdir(parents=True)
        (ds / "metadata.json").write_text(json.dumps({
            "id": "3d/Demo", "name": "Demo", "type": "3d", "configured": True,
    ...
    def test_hidden_dataset_excluded_from_catalog(self):
        ...
        ok = dev_server._set_dataset_hidden("3d/Demo", True)
        self.assertTrue(ok)
