...
echo "path allowlist\n";
$allowed = [
    ['3d', 'metadata.json'], ['3d', 'thumbnail.webp'], ['3d', 'bricks/manifest.json'],
    ['3d', 'bricks/lod0/c0/pack_00.bin'], ['3d', 'bricks/lod3/rgba/pack_12.bin'],
    ['3d', 'download/orig.ims'], ['live', 'bricks/t000/lod2/c0/pack_00.bin'],
...
$refused = [
    ...
    ['2d', 'bricks/manifest.json'], ['2d', 'bricks/lod0/c0/pack_00.bin'],
    ['2d', 'model.glb'],
...
$plan = lumen_up_plan([['type' => '3d', 'folder' => 'DS', 'files' => [
...
foreach ($blobs as $rel => $data) {
    [$st, ] = lumen_up_write_chunk('3d', 'DS', $rel, 0, $data, hash('sha256', $data));
    check("chunk $rel", $st === 200);
    [$st, ] = lumen_up_finalize('3d', 'DS', $rel, null);
    check("finalize $rel", $st === 200);
}
$v = lumen_up_validate_dataset('3d', 'DS');
check('validate ok', !empty($v['ok']));
check('state = staged', lumen_up_state_of('3d', 'DS') === LUMEN_UP_STATE_STAGED);
...
$bigLen = (int)($CS * 2.5);
lumen_up_plan([['type' => '3d', 'folder' => 'DS2', 'files' => [
    ['path' => 'bricks/lod0/c0/pack_00.bin', 'size' => $bigLen],
]]], $CS);
$whole = str_repeat('B', $bigLen);
$c0 = substr($whole, 0, $CS);
[$st, $pl] = lumen_up_write_chunk('3d', 'DS2', 'bricks/lod0/c0/pack_00.bin', 0, $c0, str_repeat('0', 64));
check('tampered chunk refused (422)', $st === 422 && $pl['error'] === 'checksum_mismatch');
[$st, $pl] = lumen_up_write_chunk('3d', 'DS2', 'bricks/lod0/c0/pack_00.bin', 0, 'AAA', hash('sha256', 'AAA'));
check('short chunk refused (400)', $st === 400 && $pl['error'] === 'bad_chunk_length');
[$st, ] = lumen_up_write_chunk('3d', 'DS2', '../../api/admin_credential.json', 0, 'x', hash('sha256', 'x'));
check('traversal refused (400)', $st === 400);
[$st, ] = lumen_up_finalize('3d', 'DS2', 'bricks/lod0/c0/pack_00.bin', null);
...
foreach ([2, 0] as $i) {
    ...
    [$st, ] = lumen_up_write_chunk('3d', 'DS2', 'bricks/lod0/c0/pack_00.bin', $i, $part, hash('sha256', $part));
    check("out-of-order chunk $i", $st === 200);
}
$replan = lumen_up_plan([['type' => '3d', 'folder' => 'DS2', 'files' => [
...
lumen_up_write_chunk('3d', 'DS2', 'bricks/lod0/c0/pack_00.bin', 1, $part, hash('sha256', $part));
[$st, ] = lumen_up_finalize('3d', 'DS2', 'bricks/lod0/c0/pack_00.bin', null);
check('finalize after resume', $st === 200);
check('bytes on disk match', file_get_contents(lumen_up_dataset_dir('3d', 'DS2') . '/bricks/lod0/c0/pack_00.bin') === $whole);

// A pack shorter than the manifest claims must block the publish.
$v2 = lumen_up_validate_dataset('3d', 'DS2');
...
echo "metadata lock\n";
[$st, ] = lumen_up_write_metadata('3d', 'DS', ['name' => 'Renamed by operator']);
check('save metadata', $st === 200);
check('name persisted', (lumen_up_read_metadata('3d', 'DS')['name'] ?? '') === 'Renamed by operator');
$replan = lumen_up_plan([['type' => '3d', 'folder' => 'DS', 'files' => [
...
[$st, $pl] = lumen_up_write_chunk('3d', 'DS', 'metadata.json', 0, $mb, hash('sha256', $mb));
check('a re-sent metadata chunk is a no-op', $st === 200 && ($pl['skipped'] ?? '') === 'locked');
check('operator name still intact', (lumen_up_read_metadata('3d', 'DS')['name'] ?? '') === 'Renamed by operator');
...
lumen_up_write_metadata('3d', 'DS', [
    'name' => 'Renamed by operator', 'staging' => true, 'stagingState' => 'editable',
    'stagingEditable' => true, 'path' => 'staging:3d/DS', 'totalBytes' => 999,
    'receivedBytes' => 999, 'publishedExists' => false, 'expiresInS' => 42, 'key' => '3d/DS',
]);
$onDisk = lumen_up_read_json(lumen_up_dataset_dir('3d', 'DS') . '/metadata.json');
...
check('landed in DATA_WEB', is_file($root . '/DATA_WEB/3d/DS/metadata.json'));
check('published hidden by default', !empty(json_decode((string)file_get_contents($root . '/DATA_WEB/3d/DS/metadata.json'), true)['hidden']));
check('staging dir cleared', !is_dir(lumen_up_staging() . '/3d/DS'));
check('journal cleared', !is_file(lumen_up_journal_path('3d', 'DS')));
[$st, $pl] = lumen_up_publish('3d', 'DS2', false, true);
...
foreach (['metadata.json' => $mb, 'bricks/manifest.json' => $nb, 'bricks/lod0/c0/pack_00.bin' => $pack] as $rel => $data) {
    lumen_up_write_chunk('3d', 'RP', $rel, 0, $data, hash('sha256', $data));
    lumen_up_finalize('3d', 'RP', $rel, null);
}
$rp2 = $rp; array_pop($rp2[0]['files']);
lumen_up_plan($rp2, LUMEN_UP_DEFAULT_CHUNK);
$j = lumen_up_load_journal('3d', 'RP');
check('unfinished + absent file pruned on re-plan', !isset($j['files']['bricks/lod9/c0/pack_00.bin']));
check('RP now validates', !empty(lumen_up_validate_dataset('3d', 'RP')['ok']));
...
$jp = lumen_up_journal_path('3d', 'RP');
...
check('state is staged', lumen_up_state_of('3d', 'RP') === LUMEN_UP_STATE_STAGED);
$g = lumen_up_gc();
check('gc spares a finished dataset', !in_array('3d/RP', $g['removed'], true) && is_dir(lumen_up_dataset_dir('3d', 'RP')));
check('no expiry countdown on a finished dataset', lumen_up_describe('3d', 'RP')['expiresInS'] === null);
...
lumen_up_ensure_dirs();
@mkdir(lumen_up_staging() . '/3d/PY/bricks/lod0/c0', 0777, true);
file_put_contents(lumen_up_staging() . '/3d/PY/bricks/lod0/c0/pack_00.bin', str_repeat("\0", 24));
file_put_contents(lumen_up_journal_path('3d', 'PY'), json_encode([
    'version' => 1, 'type' => '3d', 'folder' => 'PY',
...
// Same file name on both backends, or an import started under Python never
// resumes here: one vocabulary, no alias on either side.
check('journal file name carries the canonical type',
      basename(lumen_up_journal_path('3d', 'PY')) === '3d__PY.json'
      && basename(lumen_up_journal_path('2d', 'WM')) === '2d__WM.json');
$e = lumen_up_load_journal('3d', 'PY')['files']['bricks/lod0/c0/pack_00.bin'];
